...
function pp_invoice_the_content( $content ) {
	...
	if(!pp_invoice_md5_to_invoice($_GET['invoice_id'])) return  $content; else $invoice_id = pp_invoice_md5_to_invoice($_GET['invoice_id']);
	...
	$result .= "<div id=\"invoice_page\" class=\"clearfix\">";
		...
		if( !$stop ) { // The following is only displayed if the invoice has not been paid
			...
			pp_invoice_update_log($invoice_id,'visited',"Viewed by $ip");

			//If this is not recurring invoice, show regular message
			if(!pp_invoice_recurring($invoice_id))  pp_invoice_show_invoice_overview($invoice_id);

			// Show this if recurring
			if(pp_invoice_recurring($invoice_id))  pp_invoice_show_recurring_info($invoice_id);

			//Billing Business Address
			if(get_option('pp_invoice_show_business_address') == 'yes') pp_invoice_show_business_address();

			//Show Billing Information
			pp_invoice_show_billing_information($invoice_id);
	...
	if(get_option('pp_invoice_where_to_display') == 'overwrite' || get_option('pp_invoice_where_to_display') == '' || get_option('pp_invoice_where_to_display') == 'replace_tag') return $result;
	if(get_option('pp_invoice_where_to_display') == 'bellow_content') return $content . $result;
	if(get_option('pp_invoice_where_to_display') == 'above_content') return $result . $content;

	} else return $content;

}

function pp_invoice_frontend_header() {
if(pp_invoice_md5_to_invoice($_GET['invoice_id']) && get_option('pp_invoice_web_invoice_page') != '' && is_page(get_option('pp_invoice_web_invoice_page')))  {

$invoice_id = pp_invoice_md5_to_invoice($_GET['invoice_id']);
?>
...
<?php
function pp_invoice_curPageURL() {
 $pageURL = 'http';
 ...
 link_id = 'wp_cc_response';
	var req = jQuery.post( ajaxurl , jQuery('#checkout_form').serialize(), function(html){
		alert('In process_cc_checkout jQuery.post.');
			...
			jQuery('#pp_invoice_process_wait span').html('');
			req = null;
		}
	);
}
...
<?php if(get_option('pp_invoice_use_css') == 'yes') { ?>
#invoice_page {text-align: left; clear:both;}
#invoice_page #billing_overview #pp_invoice_select_payment_method {padding-bottom: 20px;}
...
#invoice_page #pp_invoice_select_payment_method select {width: 235px; border:0; background: #EFEFEF; padding: 7px;  -moz-border-radius: 5px;  border-radius: 5px; }
...
#invoice_page #pp_invoice_process_wait {height: 32px;}
#invoice_page #invoice_overview {width: 400px; float:left; padding-right: 10px; margin-right: 15px; position: relative;}
#invoice_page #pp_invoice_itemized_table {width: 100%; margin-bottom:10px;}
#invoice_page #pp_invoice_itemized_table .alt_row {background: #EFEFEF}
#invoice_page #pp_invoice_itemized_table .grand_total {font-weight:bold;}
#invoice_page #pp_invoice_itemized_table .description_text {color: #9F9F9F}
#invoice_page #pp_invoice_itemized_table th {background: #DFDFDF}
#invoice_page #pp_invoice_itemized_table td, #pp_invoice_itemized_table th {padding: 5px; text-align: left;}
#invoice_page #pp_invoice_itemized_table .pp_invoice_bottom_line td {border-top:1px solid #DFDFDF;}
...
#invoice_page .pp_invoice_due_date {position: absolute; top:0;right:0;margin:0;padding:0}
